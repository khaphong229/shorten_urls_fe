...
import { Space, Input, Divider, Button, Table, Tag, notification, Spin, Popconfirm } from "antd";
import { BulbOutlined, DeleteOutlined, EditOutlined } from "@ant-design/icons";
import BoxComponent from "../components/BoxComponent";
import { getAllApiKey, editStatusApiKey, deleteApiKey, searchApiKey } from "../services/apiKey";
...
function ApiLink() {
  ...
  const editStatus = useCallback(async (route, data) => {
    try {
      ...
      if (res.data.success) {
        showNotification("success", "Thành công", "Cập nhật trạng thái thành công");
        fetchApiKey();
      } else {
        showNotification("warning", "Thất bại", res.data.message);
      }
      ...
      render: (_, record) => (
        <Space size="middle">
          <Tag
            ...
            onClick={() => editStatus(`/apikeys/${record.key}`, { is_active: !record.is_active })}
    ...
    <>
      ...
      {location.pathname === "/link" && (
        <>
          ...
          <Divider />
          <BoxComponent>
            <Spin spinning={loading} tip="Đang lấy danh sách api key...">
